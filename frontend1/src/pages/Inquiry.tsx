...
const Inquiry = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setInquiryId('INQ' + Math.floor(Math.random() * 10000).toString().padStart(4, '0'));
    }, 2000);
  };
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-12">
        <h1 className="text-3xl font-bold mb-8 text-center">{t('inquiry.title')}</h1>
          ...
          <div className="p-6 md:p-8">
            {submitStatus === 'idle' ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      <label htmlFor="phone" className="block text-sm font-medium text-gray-700 mb-1">
                        {t('inquiry.form.phone')} <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="tel"
                        id="phone"
                    ...
                    <div>
                      <label htmlFor="subject" className="block text-sm font-medium text-gray-700 mb-1">
                        {t('inquiry.form.subject')} <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="text"
                        id="subject"
                        ...
                        onChange={handleInputChange}
                        className={`w-full px-3 py-2 border ${formErrors.subject ? 'border-red-500' : 'border-gray-300'} rounded-md focus:outline-none focus:ring-accent focus:border-accent`}
                      />
                    ...
                    <div className="md:col-span-2">
                      ...
                      <textarea
                        ...
                        onChange={handleInputChange}
                        rows={5}
                        className={`w-full px-3 py-2 border ${formErrors.message ? 'border-red-500' : 'border-gray-300'} rounded-md focus:outline-none focus:ring-accent focus:border-accent`}
                      />
                    ...
                    <div className="md:col-span-2">
                      ...
                      <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                        <div className="space-y-1 text-center">
                          ...
                          <p className="text-xs text-gray-500">{t('inquiry.form.fileTypes')}</p>
                          {formData.file && (
                            <p className="text-sm text-gray-600 mt-2">
                              {t('inquiry.form.selectedFile')}: {formData.file.name}
                            </p>
                          )}
                        </div>
                      </div>
              ...
              <div className="text-center py-12">
                ...
                <h2 className="text-xl font-semibold mb-2">{t('inquiry.success.title')}</h2>
                <p className="text-gray-600 mb-2">{t('inquiry.success.message')}</p>
                {inquiryId && (
                  <p className="text-gray-600 mb-6">
                    {t('inquiry.success.inquiryId')}: <span className="font-semibold">{inquiryId}</span>
                  </p>
                )}
                <button
                  onClick={resetForm}
              ...
              <div className="text-center py-12">
                ...
                <h2 className="text-xl font-semibold mb-2">{t('inquiry.error.title')}</h2>
                <p className="text-gray-600 mb-6">{t('inquiry.error.message')}</p>
                <button
